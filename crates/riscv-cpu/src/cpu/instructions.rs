use super::{
    decode_privilege_mode, Cpu, PrivilegeMode, Trap, TrapType, Xlen, CSR_MEPC_ADDRESS,
...
pub struct Instruction {
    ...
    pub disassemble: fn(cpu: &Cpu, word: u32, address: u64, evaluate: bool) -> String,
}

pub const INSTRUCTION_NUM: usize = 116;
    ...
    [
            ...
            operation: |cpu, word, _address| {
                ...
                cpu.x[f.rd] = match cpu.xlen {
                    Xlen::Bit32 => cpu.sign_extend(
                        (((cpu.x[f.rs1] as u32 as u64) * (cpu.x[f.rs2] as u32 as u64)) >> 32)
                            as i64,
                    ),
                    Xlen::Bit64 => {
                        ((cpu.x[f.rs1] as u64 as u128).wrapping_mul(cpu.x[f.rs2] as u64 as u128)
                            >> 64) as i64
                    }
                };
                Ok(())
        ...
        Instruction {
            ...
            operation: |cpu, word, _address| {
                ...
                cpu.x[f.rd] = match cpu.xlen {
                    Xlen::Bit32 => cpu.sign_extend(
                        ((cpu.x[f.rs1] as i64).wrapping_mul(cpu.x[f.rs2] as u32 as i64) >> 32)
                            as i64,
                    ),
                    Xlen::Bit64 => {
                        ((cpu.x[f.rs1] as u128).wrapping_mul(cpu.x[f.rs2] as u64 as u128) >> 64)
                            as i64
                    }
                };
                Ok(())
